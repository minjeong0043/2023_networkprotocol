...
void L3_LLI_dataInd(uint8_t* dataPtr, uint8_t srcId, uint8_t size, int8_t snr, int16_t rssi)
{
    ...
    if (L3_checkIfanswerPdu(dataPtr))
    {
        L3_event_setEventFlag(L3_event_answerPDURcvd);
    }
    else if (L3_checkIfdemandPdu(dataPtr))
    {
        L3_event_setEventFlag(L3_event_demandPDURcvd);
    }
}
