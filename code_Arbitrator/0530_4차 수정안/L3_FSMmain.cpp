...
static uint8_t quizStart_Sdu[] = "QnA protocol Start!! If you want to enjoy it, please input 'Y' in your screen.";
...
{  
    ...
    switch (main_state)
    {
        case L3STATE_Collect_Participants:
            ...
            else if(L3_event_checkEventFlag(L3_event_demandPDURcvd))
            {
                ...
                if(partiNum >= 2)
                {
                    ...
                    //partiConfirmed Pdu
                    pduSize = L3_MSG_encode_PARTICONFIRMED(partiConfirmed_Pdu, partiConfirmed_Sdu, Len_partiConfirmed);
                    ...
                    //collectComplete Pdu
                    pduSize = L3_MSG_encode_COLLECTCOMPLETE(collectComplete_Pdu, collectComplete_Sdu, Len_collectComplete);
        ...
        case L3STATE_Question_Transmission:
            ...
            {
                L3_MSG_encode_QUESTION(pdu, answer_id, wordLen, originalWord);
        ...
        case L3STATE_Answer_Transmission:
            ...
            {// Q) PDU에서 SrcId 어떻게 뽑아와?
                ...
                if (answer_id == srcId)
                {
                    ...
                    uint8_t total_Sdu[] = sdu;//Q) 꼭 필요한지.
