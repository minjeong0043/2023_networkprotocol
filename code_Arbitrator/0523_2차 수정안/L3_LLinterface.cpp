...
//Quiz start PDU전송 event 정의 필요
//L3_event_ComPartiCollect 정의 필요 partiNum==2
#if 0
void L3_event_questSDUinput(uint8_t res)
{
     //msg header setting
    //QuestionLen 정의해야하는가
    pc.printf("input your Quiz: ");
    strcpy((char*)QuestionSDU, (char*)originalWord);
    debug("[L3] msg length : %i\n", wordLen);
    wordLen = 0;
    return QuestionSDU;
}
#endif
void L3_event_answerIDinput(uint8_t res)
{
    pc.printf("input answer ID: ");
    strcpy((char*)AnswerID, (char*)originalWord);
    debug("[L3] msg length : %i\n", wordLen);
    wordLen = 0;
    return AnswerID;
}
